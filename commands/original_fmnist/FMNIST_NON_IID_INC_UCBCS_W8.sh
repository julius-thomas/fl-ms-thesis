...
# UCB candidate sampling + stoch final selection on Fashion-MNIST pathological non-IID incremental (hard blur) drift,
# with a sliding-window UCB of 8 rounds: any client not pulled in the last 8
# rounds is treated as "unvisited" and force-included in the next candidate pool.
: "${SEED:=42}"
: "${EXP_SUFFIX:=}"

python3 main.py \
    --exp_name "FMNIST_NON_IID_INC_UCBCS_W8${EXP_SUFFIX}" --seed ${SEED} --device cuda \
