...
# Cluster (CUDA) explore: sliding-window UCB candidate sampling on Fashion-MNIST, pathological non-IID,
# sudden drift (rounds 40-60). W=8 rounds. Pathological partitioning (matching the MNIST setup).
: "${SEED:=42}"
: "${EXP_SUFFIX:=}"

python3 main.py \
    --exp_name "FMNIST_NON_IID_SUD_UCBCS_W8${EXP_SUFFIX}" --seed ${SEED} --device cuda \
    ...
    --candidate_sampling ucb --ucb_c 1.0 --ucb_variant sliding --ucb_window 8 \
    --max_grad_norm 10 \
