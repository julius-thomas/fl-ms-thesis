...
# Max client selection (active sampling, uniform candidate set) on Fashion-MNIST pathological non-IID incremental (hard blur) drift.
# Candidate set big: C=0.4 (40/100 candidates), sampling_fraction=0.25 → train 10 highest-loss per round.
: "${SEED:=42}"
: "${EXP_SUFFIX:=}"

python3 main.py \
    --exp_name "FMNIST_NON_IID_INC_MAXCS${EXP_SUFFIX}" --seed ${SEED} --device cuda \
