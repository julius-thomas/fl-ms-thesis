...
# Max client selection (active sampling, uniform candidate set) on Fashion-MNIST Dirichlet non-IID sudden drift.
# Candidate set big: C=0.4 (40/100 candidates), sampling_fraction=0.25 → train 10 highest-loss per round.
: "${SEED:=42}"
: "${EXP_SUFFIX:=}"

python3 main.py \
    --exp_name "FMNIST_NON_IID_SUD_MAXCS${EXP_SUFFIX}" --seed ${SEED} --device cuda \
    --dataset FashionMNIST \
    --split_type diri --test_size 0.2 --cncntrtn 0.3 --mincls 2 \
