#!/bin/bash
# CheXpert FL — Pathological non-IID (min 2 classes per client)
# ResNet18 (hidden_size=32), 224x224 input, 25% dataset, 3 local epochs

python3 main.py \
    --exp_name "FedAvg_CheXpert_ResNet18_Patho" --seed 42 --device cuda \
    --dataset CheXpert --data_path ./external-data \
    --split_type patho --test_size 0.2 --rawsmpl 0.25 --mincls 2 \
