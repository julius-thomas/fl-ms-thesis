# UCB-CS-LIT (Cho, Gupta, Joshi, Yagan 2020 - Algorithm 1) baseline on CheXpert
# custom non-IID with custom (frontal -> lateral) sudden drift at round 100.
# Paper-aligned settings: K=100, C=0.03 (m=3 trained per round), gamma=0.9.
: "${SEED:=42}"
: "${EXP_SUFFIX:=}"

python3 main.py \
    --exp_name "CHEXPERT_NON_IID_SUD_UCBCS_LIT${EXP_SUFFIX}" --seed ${SEED} --device cuda --bf16 \
    ...
    --algorithm ucbcs_lit --eval_fraction 1 --eval_type both --eval_every 1 --eval_metrics mlacc mlauroc \
    --K 100 --R 150 --E 1 --C 0.03 --B 64 --beta1 0 \
