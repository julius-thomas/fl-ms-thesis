...
# UCB candidate sampling + max final selection on MNIST non-IID incremental (hard blur) drift.
# Candidate set big: C=0.4 (40/100 via UCB1), sampling_fraction=0.25 → train 10 highest-loss per round.
: "${SEED:=42}"
: "${EXP_SUFFIX:=}"

python3 main.py \
    --exp_name "MNIST_NON_IID_INC_UCBCS${EXP_SUFFIX}" --seed ${SEED} --device cuda \
    --dataset MNIST \
