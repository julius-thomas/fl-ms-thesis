...
# Plain UCB candidate sampling, low exploration: c=0.5.
: "${SEED:=42}"
: "${EXP_SUFFIX:=}"

python3 main.py \
    --exp_name "MNIST_NON_IID_SUD_UCBCS_C05${EXP_SUFFIX}" --seed ${SEED} --device cuda \
    --dataset MNIST \
    ...
    --candidate_sampling ucb --ucb_c 0.5 --ucb_variant plain \
    --max_grad_norm 10 \
