...
# Discounted UCB candidate sampling, high discounting (low memory): gamma=0.80.
# Effective sample size 1/(1-gamma) = 5 rounds, matched to W=5.
: "${SEED:=42}"
: "${EXP_SUFFIX:=}"

python3 main.py \
    --exp_name "MNIST_NON_IID_SUD_UCBCS_G80${EXP_SUFFIX}" --seed ${SEED} --device cuda \
    --dataset MNIST \
    ...
    --candidate_sampling ucb --ucb_c 1.0 --ucb_variant discounted --ucb_gamma 0.80 \
    --max_grad_norm 10 \
