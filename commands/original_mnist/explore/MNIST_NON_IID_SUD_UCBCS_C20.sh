...
# Plain UCB candidate sampling, high exploration: c=2.0.
: "${SEED:=42}"
: "${EXP_SUFFIX:=}"

python3 main.py \
    --exp_name "MNIST_NON_IID_SUD_UCBCS_C20${EXP_SUFFIX}" --seed ${SEED} --device cuda \
    --dataset MNIST \
    ...
    --candidate_sampling ucb --ucb_c 2.0 --ucb_variant plain \
    --max_grad_norm 10 \
