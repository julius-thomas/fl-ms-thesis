...
# Sliding-window UCB candidate sampling, high memory: W=12 rounds.
: "${SEED:=42}"
: "${EXP_SUFFIX:=}"

python3 main.py \
    --exp_name "MNIST_NON_IID_SUD_UCBCS_W12${EXP_SUFFIX}" --seed ${SEED} --device cuda \
    --dataset MNIST \
    ...
    --candidate_sampling ucb --ucb_c 1.0 --ucb_variant sliding --ucb_window 12 \
    --max_grad_norm 10 \
