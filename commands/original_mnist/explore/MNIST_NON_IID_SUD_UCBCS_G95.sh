...
# Discounted UCB candidate sampling, low discounting (high memory): gamma=0.95.
# Effective sample size 1/(1-gamma) ~= 20 rounds (slower than drift recovery).
: "${SEED:=42}"
: "${EXP_SUFFIX:=}"

python3 main.py \
    --exp_name "MNIST_NON_IID_SUD_UCBCS_G95${EXP_SUFFIX}" --seed ${SEED} --device cuda \
    --dataset MNIST \
    ...
    --candidate_sampling ucb --ucb_c 1.0 --ucb_variant discounted --ucb_gamma 0.95 \
    --max_grad_norm 10 \
