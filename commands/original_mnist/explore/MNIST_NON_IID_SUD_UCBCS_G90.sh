...
# Discounted UCB candidate sampling, standard discounting: gamma=0.90.
# Effective sample size 1/(1-gamma) ~= 10 rounds, matched to drift recovery.
: "${SEED:=42}"
: "${EXP_SUFFIX:=}"

python3 main.py \
    --exp_name "MNIST_NON_IID_SUD_UCBCS_G90${EXP_SUFFIX}" --seed ${SEED} --device cuda \
    --dataset MNIST \
    ...
    --candidate_sampling ucb --ucb_c 1.0 --ucb_variant discounted --ucb_gamma 0.90 \
    --max_grad_norm 10 \
