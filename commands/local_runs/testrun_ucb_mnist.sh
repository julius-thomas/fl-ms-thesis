...
# Local smoke test: UCB candidate sampling + max final selection on MNIST non-IID sudden drift.
# Uses TwoNN (smaller than TwoCNN) and mps for Mac-local iteration.
python3 main.py \
    --exp_name MNIST_NON_IID_SUD_UCBCS_TEST --seed 42 --device mps \
    --dataset MNIST \
